...
async function loadReservations() {
  try {
    ...
    const recordsWithApproval = records.filter((r) => r.facultyInChargeApprove);
    console.log(
      "Records with facultyInChargeApprove:",
    ...
    records.forEach((rec) => {
      ...
      console.log(
        `Record ${rec.id} facultyInChargeApprove:`,
        rec.facultyInChargeApprove
      );

      // Display property custodian approval status from database
      let approvalStatus = "";
      if (rec.facultyInChargeApprove && rec.facultyInChargeApprove !== "N/A") {
        let badgeClass = "bg-secondary";
        const approvalText = rec.facultyInChargeApprove.toLowerCase();
        ...
        approvalStatus = `<span class="badge ${badgeClass} ms-2">${rec.facultyInChargeApprove}</span>`;
      } else {
...
async function handleSaveApprovals() {
  try {
    ...
    let facultyInChargeApprove = "Under Review";
    if (allApproved) {
      facultyInChargeApprove = "Approved";
    } else if (hasRejected) {
      facultyInChargeApprove = "Rejected";
    }

    // Prepare the update payload
    const updateData = {
      facultyInChargeApprove: facultyInChargeApprove,
    ...
    currentApprovals.set(reservationId, {
      approvals,
      timestamp: new Date().toISOString(),
      approvedBy: "faculty_in_charge",
      dbStatus: facultyInChargeApprove,
    });
...
async function debugReservationData(reservationId) {
  try {
    ...
    console.log("facultyInChargeApprove field:", record.facultyInChargeApprove);
    console.log(
