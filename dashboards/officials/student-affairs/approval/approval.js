...
async function loadReservations() {
  try {
    const records = await pb.collection("reservation").getFullList({
      ...
      filter: 'eventType = "organization" && status != "approved"',
    });
    ...
    const recordsWithApproval = records.filter(
      (r) => r.headOfStudentAffairsApprove
    );
    console.log(
      "Records with headOfStudentAffairsApprove:",
    ...
    records.forEach((rec) => {
      ...
      console.log(
        `Record ${rec.id} headOfStudentAffairsApprove:`,
        rec.headOfStudentAffairsApprove
      );
      ...
      if (
        rec.headOfStudentAffairsApprove &&
        rec.headOfStudentAffairsApprove !== "N/A"
      ) {
        let badgeClass = "bg-secondary";
        const approvalText = rec.headOfStudentAffairsApprove.toLowerCase();
        ...
        approvalStatus = `<span class="badge ${badgeClass} ms-2">${rec.headOfStudentAffairsApprove}</span>`;
      } else {
...
async function handleSaveApprovals() {
  try {
    ...
    let headOfStudentAffairsApprove = "Under Review";
    if (allApproved) {
      headOfStudentAffairsApprove = "Approved";
    } else if (hasRejected) {
      headOfStudentAffairsApprove = "Rejected";
    }

    // Prepare the update payload
    const updateData = {
      headOfStudentAffairsApprove: headOfStudentAffairsApprove,
      approvalDetails: JSON.stringify({
        approvals,
        timestamp: new Date().toISOString(),
        approvedBy: "head_of_student_affairs",
      }),
    };
    ...
    currentApprovals.set(reservationId, {
      ...
      timestamp: new Date().toISOString(),
      approvedBy: "head_of_student_affairs",
      dbStatus: headOfStudentAffairsApprove,
    });
...
async function debugReservationData(reservationId) {
  try {
    ...
    console.log(
      "headOfStudentAffairsApprove field:",
      record.headOfStudentAffairsApprove
    );
    console.log(
