...
async function loadReservations() {
  try {
    ...
    console.log(
      "Records with headOfAcademicProgramsApprove:",
    ...
    records.forEach((rec) => {
      ...
      console.log(
        `Record ${rec.id} headOfAcademicProgramsApprove:`,
        rec.headOfAcademicProgramsApprove
      );
      ...
      if (
        rec.headOfAcademicProgramsApprove &&
        rec.headOfAcademicProgramsApprove !== "N/A"
      ) {
        let badgeClass = "bg-secondary";
        const approvalText = rec.headOfAcademicProgramsApprove.toLowerCase();
        ...
        approvalStatus = `<span class="badge ${badgeClass} ms-2">${rec.headOfAcademicProgramsApprove}</span>`;
      } else {
...
async function handleSaveApprovals() {
  try {
    ...
    let headOfAcademicProgramsApprove = "Under Review";
    if (allApproved) {
      headOfAcademicProgramsApprove = "Approved";
    } else if (hasRejected) {
      headOfAcademicProgramsApprove = "Rejected";
    }

    // Prepare the update payload
    const updateData = {
      headOfAcademicProgramsApprove: headOfAcademicProgramsApprove,
    ...
    currentApprovals.set(reservationId, {
      approvals,
      timestamp: new Date().toISOString(),
      approvedBy: "Head of Academic Programs",
      dbStatus: headOfAcademicProgramsApprove,
    });
...
async function handleApprovalWorkflow(
  ...
  try {
    ...
    const activityLog = {
      ...
      action: "head_of_academics_approval",
...
async function debugReservationData(reservationId) {
  try {
    ...
    console.log(
      "campusDirectorApprove field:",
      record.headOfAcademicProgramsApprove
    );
    console.log(
