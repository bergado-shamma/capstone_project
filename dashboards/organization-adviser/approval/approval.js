...
async function loadReservations() {
  try {
    ...
    const recordsWithApproval = records.filter(
      (r) => r.organizationAdviserApprove
    );
    console.log(
      "Records with organizationAdviserApprove:",
    ...
    records.forEach((rec) => {
      ...
      console.log(
        `Record ${rec.id} organizationAdviserApprove:`,
        rec.organizationAdviserApprove
      );
      ...
      if (
        rec.organizationAdviserApprove &&
        rec.organizationAdviserApprove !== "N/A"
      ) {
        let badgeClass = "bg-secondary";
        const approvalText = rec.organizationAdviserApprove.toLowerCase();
        ...
        approvalStatus = `<span class="badge ${badgeClass} ms-2">${rec.organizationAdviserApprove}</span>`;
      } else {
...
async function handleSaveApprovals() {
  try {
    ...
    let organizationAdviserApprove = "Under Review";
    if (allApproved) {
      organizationAdviserApprove = "Approved";
    } else if (hasRejected) {
      organizationAdviserApprove = "Rejected";
    }

    // Prepare the update payload
    const updateData = {
      organizationAdviserApprove: organizationAdviserApprove,
    ...
    currentApprovals.set(reservationId, {
      approvals,
      timestamp: new Date().toISOString(),
      approvedBy: "organization_adviser",
      dbStatus: organizationAdviserApprove,
    });
...
async function debugReservationData(reservationId) {
  try {
    ...
    console.log(
      "organizationAdviserApprove field:",
      record.organizationAdviserApprove
    );
    console.log(
